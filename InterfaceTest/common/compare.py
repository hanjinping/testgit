...
class CompareParam(object):
    ...
    def compare_code(self,result_interface):
        ...
        try:
            if result_interface.startswith('{') and isinstance(result_interface,str):
                temp_result_interface=json.loads(result_interface)  #将字符类型转化为字典类型
                temp_code_to_compare=self.params_interface['code_to_compare']  #获取待比较的code名称
    ...
    def compare_params_complete(self,result_interface):
        ...
        try:
            ...
            if temp_compare_params['code']=='0000':
                ...
                if self.params_to_compare.startswith('{') and isinstance(self.params_to_compare,list): #判断测试用例表中预期结果集是否为列表
    ...
    #定义递归方法，将接口返回数据中的参数名写入列表中（去重）
    def __recur_params(self,result_interface):
        try:
            if result_interface.startswith('{') and isinstance(result_interface,str):
                temp_result_interface=json.loads(result_interface)  #入参是字符类型，且能被转化为字典类型
