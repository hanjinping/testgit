...
def read_testsuite(tsname):
    ...
    if os.path.exists(tsname):
        ...
        for irow in range(2,ws.max_row+1):
            ...
            # 判断执行标志位是否需要执行，如果是do,则调用测试用例执行函数
            # 如果是not,则不执行，如果是其他值，则写入日志，报告执行参数错误，
            # 并指出是哪个用例执行参数错误
            if testoperation == "do":
                logger.info("*****************************")
...
# 定义浏览器启动函数，本次未使用common中定义的browserstart函数
    ...
    if os.path.exists(testcasefile):
        ...
        for irow in range(2,ws.max_row+1):
            ...
            if irow==2:
                ...
                # 如果不是浏览器，则说明需要执行测试用例，调用测试用例执行函数
